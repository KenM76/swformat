...
| `swXmlContents/COMPINSTANCETREE` | `api.components` (`read_component_tree`) + `swf-comptree` | M4 | **SHIPPED (READ)** — assembly component tree: per `<swReference>` instance → name, resolved path (`swModelRef`→`swModel`→`swFile.swPath` join), referenced config, and **exclude-from-BOM / flexible / hidden / suppressed / virtual** flags + `swTransform` (and `swBoundingBox`, resolved from the component's `swModel` when SW stored it). One read replaces a per-component COM loop (`GetExcludeFromBOM2` + `GetParent`/`GetPathName`). **READ-only MIRROR** — `swExcludeFromBOM` verified to flip NO→YES on exclude (twin diff-pair, SW 2026); editing it alone is NOT SW-clean (authoritative store is `Contents/CMgr`; a COMPINSTANCETREE-only edit hung the load). `swObjCount` grows when an instance references a different config. |
...
| `swXmlContents/Features` | `api.components` (`read_part_config_tree`) + `swf-comptree` (part) | — | **SHIPPED (READ)** — a PART's identity + config list (analog of COMPINSTANCETREE): file path, doc type, creation time, configuration names + most-recent/active. |
| `docProps/Config-N-Cutlist-Properties.xml` | `api.cutlist_xml` (`read_cutlist_xml`) + `swf-cutlist` | — | **SHIPPED (READ)** — weldment cut-list items per config: feature name/type, `CustomProperty` values, **`ExcludeFromCutlist`**, `Quantity`, `CutlistType`. Read mirror of the binary cut-list store (which has SHIPPED value-editing via `carchive.cusprops`). |
