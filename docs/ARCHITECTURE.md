...
| `docProps/core.xml` | `CorePropsStream` (read-only initially) | M2 | pending |
...
| `swXmlContents/COMPINSTANCETREE` | `CompInstanceTreeStream` | M4 | **OBSERVED** — component-instance tree (XML). A MIRROR of the config-manager state, NOT authoritative: editing it alone does not change a flexible instance's config (SW regenerates it from `Contents/CMgr`). `swObjCount` GROWS when an instance references a different config (15→18 in the flexible diff-pair). Read handler pending. |
